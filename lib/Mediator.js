...
module.exports = class CommandLineMediator extends Mediator {
	...
	_releaseWorkSpace () {
		...
		this._terminals.forEach((terminal) => {
			...
			terminal.commandlines.forEach((commandline) => {
	...
	// public
		...
		getAllTerminals () {
			...
			return Promise.resolve(this._terminals.map((terminal) => {

				return {
					"number": terminal.number,
					"name": terminal.name,
					"commandlines": terminal.commandlines
				};
		...
		closeTerminal (urlParameters, bodyParameters) {
