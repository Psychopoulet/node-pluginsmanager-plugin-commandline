	...
	// public
		...
		commandLine (urlParameters, bodyParameters) {
			...
			return Promise.resolve().then(() => {
				...
				return 1 < bodyParameters.command.split(" ").length ? Promise.reject(new Error(
					"\"" + bodyParameters.command + "\" command must not contains spaces"
				)) : Promise.resolve();
				...
				return !terminal ? Promise.reject(new Error(
					...
					terminal.currentcommandline = {
						"command": bodyParameters.command,
						"arguments": bodyParameters.arguments ? bodyParameters.arguments : []
					};
				...
				terminal.child_process.stdin.write(bodyParameters.command + (
					bodyParameters.arguments ? " " + bodyParameters.arguments.join(" ") : ""
				));
